...
- Twitch/Rumble preservation: Twitch attachment UI/routes and Rumble behavior were not changed; Creator remains a thin consumer of Runtime/Auth integration state.
- Historical restore note: this is the Creator-side companion to the Runtime/Auth Kick authority restore, not a broad UI redesign.
- Human note: pressing Reconnect Kick should either send the creator into Kick authorization and return to `/integrations/kick`, or show the concrete Runtime/Auth start error.
...
- Added Creator-side platform identity alias management on `/account` using the Runtime/Auth `/api/account/platform-identities` contract. `views/account.html` now exposes a compact chat identity matching panel, `js/account-settings.js` loads aliases, validates manual alias submissions, prevents obvious duplicate empty submissions, and saves through POST/PATCH only, while `css/creator-dashboard.css` keeps the alias list/form compact and responsive. Source-level tests pin the account panel, endpoint wiring, required identifier rule, duplicate guard, and backend-error display path.
- Added compact chat identity matching hints to the platform detail controller so Kick/Rumble/Twitch/YouTube pages can show detected identifiers only when the existing integration payload exposes them and otherwise point creators to the manual alias panel. Kick hints use current runtime fields such as broadcaster user ID/channel slug/display name; Rumble copy stays limited to chat payload IDs/usernames and does not add scraping or new provider logic.
- Added a disabled advanced scoped livechat global roll-up suppression panel on `/account`. Runtime/Auth currently has storage methods for `suppress_global_rollup`, but this Creator task did not find a creator-owned read/write API route for scoped progression settings, so the setting remains OFF/disabled with a clear "Runtime setting not available yet" state. No localStorage or Creator-owned setting authority was introduced.
- Human note: creators can now add exact chat IDs/usernames/handles so livechat XP/rank/inventory matching is more reliable, while the risky channel-isolation roll-up option is visible as a future advanced backend contract rather than a working local toggle.

- Reflowed Creator `/triggers` to remove the sparse right-hand custom trigger column introduced by the previous overhaul. `views/triggers.html` now gives the built-in and planned Runtime/Auth trigger library the main full-width row, moves My Custom Triggers into its own dedicated row below, and keeps the dry-run preview as a compact full-width panel. `css/creator-dashboard.css` adds a two-column internal custom row grid for the create/edit form plus list pane, collapsing cleanly at narrower widths. Runtime/Auth authority is unchanged: protected built-ins stay read-only, custom config writes stay on `/api/livechat/custom-triggers`, validation stays on `/api/livechat/trigger-editor/validate`, and dry-run preview stays on `/api/livechat/trigger-editor/dry-run`.
- Human note: Creator `/triggers` should use the page width better now; the custom trigger editor/list no longer forces a short empty right column beside taller built-in and planned module content.

- Slimmed and visually overhauled the Creator Chat Triggers route while preserving Runtime/Auth authority. `views/triggers.html` now presents a compact status strip, protected Built-in Commands section, separate My Custom Triggers editor/list, compact Dry-run preview panel, and smaller Planned Modules roadmap instead of the older hero/card-heavy layout. Built-in/admin/system rows remain protected/read-only, creator custom trigger create/edit/toggle/delete still use `/api/livechat/custom-triggers`, validation still uses `/api/livechat/trigger-editor/validate`, and preview still uses `/api/livechat/trigger-editor/dry-run`.
- Updated `js/triggers.js` to render the Runtime/Auth editor payload into compact trigger rows, classify XP/rank, clips/FFmpeg, economy/inventory, game, built-in, and admin/system display families without adding frontend authority, and keep planned modules visibly staged/coming soon. Added scoped route styles in `css/creator-dashboard.css` for the slim status strip, two-column editor layout, protected trigger rows, roadmap rows, and tighter custom trigger form/list spacing. Focused tests now pin editor endpoint consumption, protected built-ins, separate custom triggers, staged planned modules, dry-run wiring, and no admin/system edit path.
- Human note: Creator `/triggers` should now feel like a focused creator tool: custom rows are easy to manage, built-ins are visible but protected, and preview/validation are compact and obviously no-send.

- Overhauled the Creator Chat Triggers route to consume the Runtime/Auth trigger editor contract. `js/triggers.js` now hydrates registry cards from `/api/livechat/trigger-editor`, keeps built-in/system rows protected, shows planned XP/rank/economy/game/clip module states as unavailable, validates custom trigger form payloads through `/api/livechat/trigger-editor/validate` before save, and runs the simulator through `/api/livechat/trigger-editor/dry-run` without dispatching to any transport. Custom trigger create/edit/toggle/delete still use the existing runtime/Auth `/api/livechat/custom-triggers` authority path.
- Updated `views/triggers.html` copy so Creator sees built-in commands as protected, custom triggers as creator-owned runtime config, and planned modules as coming soon/module unavailable until backend support exists. Focused source tests now pin trigger-editor hydration, validation/dry-run wiring, read-only built-in presentation, planned module copy, and no local transport bypass.
- Human note: creators should be able to inspect available built-in commands, manage only their custom rows, preview `!bot` or a custom trigger safely, and see `!clip`/XP-family roadmap commands without being told they already work.

- Extended the Creator Kick integration consumer for runtime/Auth-owned channel identity and deploy-target posture. `js/platform-integration-detail.js` now renders the redacted Kick `channel_link_saved`, slug, channel URL, broadcaster user id, identity status/error, deploy-target export, and auto-deploy reason fields; Refresh/Recheck continues to call runtime reconcile; OAuth callback `warning=channel_unresolved` shows a safe banner; and a small manual `Kick channel slug / URL` fallback posts only non-secret slug/URL data to `POST /api/creator/integrations/kick/channel`. Creator still does not store or render Kick OAuth tokens, refresh tokens, client secrets, auth codes, PKCE verifiers, cookies, or raw Authorization headers. No files were created or removed.

- Upgraded the Creator Kick integration page from a staged planning view into a runtime/Auth-owned OAuth and credential-management consumer. `views/platforms/kick.html`, `js/platform-integration-detail.js`, and `js/integrations-hub.js` now call only the runtime Kick OAuth/status/reconcile/disconnect endpoints, navigate to the returned Kick authorization URL, render redacted scopes/token/webhook/subscription/dispatch/auto-deploy/bot-session posture, and show safe next steps from runtime/Auth. Creator still does not store or render Kick access tokens, refresh tokens, client secrets, auth codes, PKCE verifiers, cookies, or raw Authorization headers. No files were created or removed.

- Repaired the Creator `/account` profile editor scroll regression in `css/creator-dashboard.css` without changing the reordered profile/media structure, profile save/upload/reset behavior, or the shared social registry. The account shell tab strip no longer uses account-specific sticky positioning, and the desktop `Core profile content` card no longer pins while later cards scroll; the account editor now stays on the existing `#app-main` scroll surface with profile/media cards flowing in document order. No files were created, removed, or renamed.
...
- Follow-up corrected the Creator polish pass in `css/creator-dashboard.css` by removing the `#app-nav` and sidebar nav-link selectors from the added theme-alignment block. This restores the previous Creator sidebar/nav item treatment while keeping non-sidebar shell/page polish intact. No files were created or removed; this is a small CSS removal and is expected to make the touched CSS shorter than the prior polish pass. Behavior is unchanged, checkbox controls, toggle switches, and the topbar loader gradient remain untouched, and individual Public `/u/{user}` profile pages remain preserved and not intentionally redesigned.
...
- Extended the Creator wheel editor corrective pass in `js/wheels.js` and `css/creator-dashboard.css` so owners now edit the authoritative custom-slug/default-slug/shortlink model, owner-only spin policy, and real center-image field with inline local-file embedding support that stays truthful to the saved wheel payload. Added `assets/icons/wheelarrow.svg`, reused the shared `assets/placeholders/wheelcenterdefault.webp` wheel center default, and updated `README.md` so the repo tree notes the new wheel asset additions. No files were removed in this follow-up.
- Corrected the Creator wheel sound preview asset origin in `js/wheels.js` so local and production preview buttons now load the real wheel audio files from the Public asset host instead of pointing at missing `/assets/sounds/wheels/...` paths inside the Creator repo. No files were added or removed in this follow-up; the touched JS/test files are only slightly longer because the sound base resolution is now explicit and locally test-covered.
- Extended `js/wheels.js` and `css/creator-dashboard.css` so each wheel sound category now has a real preview button that plays the currently selected asset immediately, stops or restarts cleanly, and does not require a save first. No files were removed or replaced in this part of the pass; the touched Creator JS/CSS files are only slightly longer because of the additive preview-state wiring.
- Added `views/leaderboards.html`, updated `js/routes.js`, `js/render.js`, `functions/[[path]].js`, `js/app.js`, `index.html`, and `README.md`, and preserved legacy `/scoreboards` as an alias to the new `/leaderboards` scaffold instead of treating it as a duplicate wheel manager route. The repo is expected to be one file longer because `views/leaderboards.html` is new and intentionally additive.
