...
- Saved profile image versions hydrate from their direct canonical, query-free Runtime/Auth URL. Exact legacy API/CDN paths normalize once to the stable same-origin Public transport; Creator does not invent alternate origins or retry loops. A failed save revokes staged object URLs, clears the affected file inputs, and restores the last authoritative profile preview so unsaved blob/data values cannot become persisted profile state.
- Creator account settings currently surface canonical slug editing and visibility, StreamSuites public profile visibility, FindMeHere listing controls, truthful dual share previews, hardened media URL previews, reserved media fields including background image URL, distinct hero Bio and Markdown About fields, an About publication toggle that preserves hidden content, an independent optional embed/upload video, separate Runtime-backed StreamSuites Dark/Light tone and feature-gradient theme settings (defaulting to Dark plus Violet Blue), grounded public social links, and a small pointer over to Preferences for moderator/community controls.
- The latest local Creator appearance-editor evidence is retained under `output/playwright/` as `creator-account-tone-editor-dark-1440.png`, `creator-account-tone-editor-light-1440.png`, `creator-account-tone-preview-light-1440.png`, and `creator-account-tone-editor-light-mobile-390.png`. These are deterministic browser-validation artifacts, not deployed profile content or live-account acceptance.
