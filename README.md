...
- The Creator sidebar brand uses the repo-local `assets/logos/ssmainlogosq.webp` square mark, with a stable 44px slot and left-aligned title/badge grouping in the expanded shell. Its title uses the same bold, full-width Tektur treatment as the Public shell, and its subheading chip uses the same centered 17px geometry; the collapsed rail remains icon-only.
