...
- The `/account` About editor can select preserved text or one Runtime/Auth-validated YouTube video/live, direct Rumble iframe URL, or Kick live-channel source. Creator sends only the chosen provider and URL, previews the authoritative response, never accepts raw iframe markup, and does not own the saved player URL.
