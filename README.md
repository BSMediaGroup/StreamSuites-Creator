...
- The `/account` Social links editor renders only currently defined destinations as compact URL detail cards. `Add link` opens an unused-platform icon menu plus a repeatable Custom link action; standard platforms stay single-instance, while up to six custom destinations may carry an optional SVG, PNG, BMP, or WebP icon immediately beneath their card and otherwise retain the fallback icon. All values continue to save through Runtime/Auth.
